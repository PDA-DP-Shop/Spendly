    ...
    <div className="min-h-dvh bg-white flex flex-col safe-top safe-bottom">
      ...
      <div className="px-7 pt-12 pb-6 flex items-center justify-between">
        ...
        <motion.button 
          whileTap={{ scale: 0.9 }}
          onClick={() => navigate(-1)}
          className="w-11 h-11 rounded-full bg-[#F6F6F6] border border-[#EEEEEE] flex items-center justify-center active:bg-gray-100"
        >
      ...
      <div className="flex-1 px-8 pt-4 overflow-y-auto scrollbar-hide">
        ...
        >
          <h1 className="text-[30px] font-[900] text-black tracking-tight leading-[1.1] mb-5" style={S}>
            Clear all <br/><span className="text-red-500">payment data?</span>
          </h1>
          ...
          <div className="p-6 rounded-[32px] bg-red-50/30 border border-red-100/50 mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-[13px] font-[802] text-red-950 uppercase tracking-wider" style={S}>Permanently Deleting</p>
            </div>
            <div className="space-y-3.5">
              {[
                'All transaction records',
                'Income & Expense history',
                'All scanned HD receipts',
                'Recurring bill reminders'
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3 text-[15px] font-[700] text-red-900/80">
                  <div className="w-1.5 h-1.5 rounded-full bg-red-500/40" />
                  <span style={S}>{item}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="p-6 rounded-[32px] bg-[#F6F6F6] border border-[#EEEEEE] mb-10">
            <p className="text-[12px] font-[802] text-[#AFAFAF] uppercase tracking-[0.15em] mb-4" style={S}>Will Stay Safe</p>
            <div className="space-y-3.5">
              {[
                'Profile & App preferences',
                'Category custom list',
                'Budget limits & Lock settings'
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3 text-[14px] font-[700] text-[#545454]">
          ...
          <div className="space-y-4 pt-2">
            <div className={`
              ...
              ${isValid ? 'border-emerald-500 bg-emerald-50/50' : error ? 'border-red-500 bg-red-50 animate-shake' : 'border-[#EEEEEE] bg-[#F6F6F6]'}
            `}>
              <input
                type="text"
                ...
                className="w-full text-center text-[20px] font-[900] tracking-[0.2em] text-black bg-transparent placeholder:text-[#D8D8D8] outline-none"
                autoComplete="off"
                autoCorrect="off"
                spellCheck="false"
                autoFocus
      ...
      <div className="p-8 pb-12 flex gap-4 bg-white sticky bottom-0">
        <motion.button
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate(-1)}
          className="flex-1 py-5 rounded-[24px] bg-[#F6F6F6] text-[#545454] font-[802] text-[16px]"
          style={S}
        >
          Go Back
        </motion.button>
        <motion.button
          ...
          className={`
            ...
            ${isValid ? 'bg-black text-white shadow-2xl shadow-black/20' : 'bg-[#EEEEEE] text-[#AFAFAF] cursor-not-allowed'}
          `}
          style={S}
        >
