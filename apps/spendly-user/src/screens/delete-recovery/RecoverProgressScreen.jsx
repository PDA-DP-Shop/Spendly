...
import { CheckCircle2, Loader2, RefreshCw, Database, ShieldCheck, Zap } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { recoveryVaultService } from '../../services/recoveryVault';

const RecoverProgressScreen = () => {
  const [steps, setSteps] = useState([
    { id: 'prep', label: 'Unlocking Secure Vault', status: 'pending', detail: 'Decrypting data blocks' },
    { id: 'restore', label: 'Restoring Transactions', status: 'pending', detail: 'Rebuilding local database' },
    { id: 'final', label: 'Verifying Integrity', status: 'pending', detail: 'Finalizing restoration' },
  ]);
  ...
  return (
    <div className="min-h-dvh bg-white flex flex-col pt-24 px-8 safe-top safe-bottom">
      <div className="flex flex-col items-center text-center mb-16">
        ...
        <p className="text-[#AFAFAF] text-[14px] font-[600] mt-1" style={S}>Finalizing local sync...</p>
      </div>
      ...
      <div className="space-y-6 flex-1 max-w-[320px] mx-auto w-full">
        {steps.map((step, i) => (
          ...
          >
            ...
            <div>
              <span className={`text-[16px] font-[802] block leading-tight ${step.status === 'done' ? 'text-black' : step.status === 'active' ? 'text-emerald-700' : 'text-[#D8D8D8]'}`} style={S}>
                {step.label}
              </span>
              <AnimatePresence>
                {step.status === 'active' && (
      ...
      <div className="pb-16 px-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-[802] text-[#AFAFAF] uppercase tracking-[0.2em]" style={S}>Data Recovery Active</span>
          </div>
